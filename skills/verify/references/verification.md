...
8. Run `soroe design --check` to confirm the pack is current.
...
A verification report is only useful if it is honest:

- A check you could not run is `blocked`, not `passed`.
- A check that partially matches is `failed` with a note on what matched and what did not.
- A manual review must name the reviewer and the decision.
